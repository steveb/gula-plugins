...
shape_mode = hslider("[01]shape_mode [name: Shape Mode][style:menu{'Random -> Sine': 0; 'Sine -> Square': 1; 'Impulse -> Square -> Pulse': 2; 'Saw (asc) -> Tri -> Saw (desc)': 3}]", 1, 0, 3, 1);
...
shape = hslider("[05]shape [name: Shape][tooltip: Wave shape for mode]", 0.0, 0.0, 1.0 ,0.01), 0.99 : min;

output = gula_lf_osc(freq, amp, shape_mode + shape) + offset;
