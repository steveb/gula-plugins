...
#ifndef  __peak_audio_to_cv_H__
#define  __peak_audio_to_cv_H__
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS peak_audio_to_cv
#endif
...
class peak_audio_to_cv : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("description", "CV output value derived from the amplitude of the input, with auto-detection of the peak aplitude");
		m->declare("filename", "peak_audio_to_cv.dsp");
	...
	virtual peak_audio_to_cv* clone() {
		return new peak_audio_to_cv();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("attack", &fHslider0, 0.00999999978f, 0.0f, 0.5f, 0.00999999978f);
		ui_interface->declare(&fHslider1, "02", "");
		ui_interface->declare(&fHslider1, "name", "Release");
		ui_interface->declare(&fHslider1, "unit", "s");
		ui_interface->addHorizontalSlider("release", &fHslider1, 0.5f, 0.0f, 2.0f, 0.00999999978f);
		ui_interface->declare(&fHslider2, "03", "");
		ui_interface->declare(&fHslider2, "name", "Peak Decay");
		ui_interface->declare(&fHslider2, "unit", "s");
		ui_interface->addHorizontalSlider("peak_decay", &fHslider2, 5.0f, 0.0f, 30.0f, 0.100000001f);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i0 = 0; (i0 < count); i0 = (i0 + 1)) {
			float fTemp0 = std::fabs(float(input0[i0]));
			float fTemp1 = ((fRec0[1] > fTemp0) ? fSlow5 : fSlow2);
			fRec1[0] = ((fRec1[1] * fTemp1) + (fTemp0 * (1.0f - fTemp1)));
			fRec0[0] = fRec1[0];
			fRec3[0] = std::max<float>(fTemp0, ((fRec3[1] * fSlow8) + (fTemp0 * fSlow9)));
...
#ifndef PLUGIN_URI
#define PLUGIN_URI URI_PREFIX "/peak_audio_to_cv"
#endif
...
struct LV2Plugin {
  ...
  static void init_meta()
  {
    if (!meta && (meta = new Meta)) {
      ...
      peak_audio_to_cv* tmp_dsp = new peak_audio_to_cv();
      if (tmp_dsp) {
  ...
  {
    return meta_get("name", "peak_audio_to_cv");
  }
      ...
      vd(num_voices>0?new VoiceData(num_voices):0)
  {
    ...
    dsp = (peak_audio_to_cv**)calloc(ndsps, sizeof(peak_audio_to_cv*));
...
#endif
  if (!plugin_name || !*plugin_name) plugin_name = "peak_audio_to_cv";
