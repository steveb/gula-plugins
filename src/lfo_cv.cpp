...
class lfo_cvSIG0 {
  ...
  public:
	...
	int getNumInputslfo_cvSIG0() {
		return 0;
	}
	int getNumOutputslfo_cvSIG0() {
		return 1;
	}
	
	void instanceInitlfo_cvSIG0(int sample_rate) {
	...
	void filllfo_cvSIG0(int count, float* table) {
...
class lfo_cv : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("description", "A CV output of an LFO with multiple shape modes and shapes");
		m->declare("filename", "lfo_cv.dsp");
	...
	static void classInit(int sample_rate) {
		lfo_cvSIG0* sig0 = newlfo_cvSIG0();
		sig0->instanceInitlfo_cvSIG0(sample_rate);
		sig0->filllfo_cvSIG0(65536, ftbl0lfo_cvSIG0);
		deletelfo_cvSIG0(sig0);
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("LFO CV");
...
#ifndef PLUGIN_URI
#define PLUGIN_URI URI_PREFIX "/lfo_cv"
#endif
...
struct LV2Plugin {
      ...
      vd(num_voices>0?new VoiceData(num_voices):0)
  {
    ...
    dsp = (lfo_cv**)calloc(ndsps, sizeof(lfo_cv*));
...
#endif
  if (!plugin_name || !*plugin_name) plugin_name = "lfo_cv";
