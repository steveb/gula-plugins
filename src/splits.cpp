 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			output0[i] = FAUSTFLOAT((fTemp0 * std::sqrt(std::min<float>(std::max<float>(((fRec0[0] > 0.0f) ? ((fTemp3 * (fRec0[0] - fTemp1)) + 1.0f) : (fTemp2 / fRec1[0])), 0.0f), 1.0f))));
			output1[i] = FAUSTFLOAT((fTemp0 * std::sqrt(std::min<float>(std::max<float>(((fRec0[0] > 1.0f) ? ((fTemp3 * (fRec0[0] + (-1.0f - fTemp1))) + 1.0f) : ((fTemp2 + -1.0f) / fRec1[0])), 0.0f), 1.0f))));
			output2[i] = FAUSTFLOAT((fTemp0 * std::sqrt(std::min<float>(std::max<float>(((fRec0[0] > 2.0f) ? ((fTemp3 * (fRec0[0] + (-2.0f - fTemp1))) + 1.0f) : ((fTemp2 + -2.0f) / fRec1[0])), 0.0f), 1.0f))));
			output3[i] = FAUSTFLOAT((fTemp0 * std::sqrt(std::min<float>(std::max<float>(((fRec0[0] > 3.0f) ? ((fTemp3 * (fRec0[0] + (-3.0f - fTemp1))) + 1.0f) : ((fTemp2 + -3.0f) / fRec1[0])), 0.0f), 1.0f))));
			fRec0[1] = fRec0[0];
			fRec1[1] = fRec1[0];
		}
	}
