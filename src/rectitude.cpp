...
#ifndef  __rectitude_H__
#define  __rectitude_H__
...
class rectitude : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("description", "Combines two signals by fully rectifying both in opposite directions");
		m->declare("filename", "rectitude.dsp");
		m->declare("license", "GPLv3");
		m->declare("name", "Rectitude");
	...
	virtual rectitude* clone() {
		return new rectitude();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("Rectitude");
		ui_interface->declare(&fHslider0, "1", "");
		ui_interface->declare(&fHslider0, "name", "Gain Up");
		ui_interface->declare(&fHslider0, "tooltip", "Gain for the up rectified signal");
		ui_interface->addHorizontalSlider("gain_up", &fHslider0, 1.0f, 0.0f, 1.0f, 0.00999999978f);
		ui_interface->declare(&fHslider1, "2", "");
		ui_interface->declare(&fHslider1, "name", "Gain Down");
		ui_interface->declare(&fHslider1, "tooltip", "Gain for the down rectified signal");
		ui_interface->addHorizontalSlider("gain_down", &fHslider1, 1.0f, 0.0f, 1.0f, 0.00999999978f);
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		for (int i0 = 0; (i0 < count); i0 = (i0 + 1)) {
			output0[i0] = FAUSTFLOAT(((fSlow0 * std::max<float>(float(input0[i0]), 0.0f)) + (fSlow1 * std::min<float>(float(input1[i0]), 0.0f))));
		}
	}
...
#ifndef PLUGIN_URI
#define PLUGIN_URI URI_PREFIX "/rectitude"
#endif
...
struct LV2Plugin {
  ...
  static void init_meta()
  {
    if (!meta && (meta = new Meta)) {
      ...
      rectitude* tmp_dsp = new rectitude();
      if (tmp_dsp) {
  ...
  {
    return meta_get("name", "rectitude");
  }
      ...
      vd(num_voices>0?new VoiceData(num_voices):0)
  {
    ...
    dsp = (rectitude**)calloc(ndsps, sizeof(rectitude*));
...
#endif
  if (!plugin_name || !*plugin_name) plugin_name = "rectitude";
